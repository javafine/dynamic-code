...
public class ConfigurationManager {
    ...
    public static String CONF_PATH_DEFAULT = "release\\conf";
    public static String CONF_DIR_DEFAULT = "conf";
    private static final Map<String, ConfigObject> MAP_CONFIG = new HashMap<String, ConfigObject>();

    private static String getConfDir() {
        ApplicationHome home = new ApplicationHome(ConfigurationManager.class);
        if (home==null || home.getSource()==null){
            //开发环境junit启动时
            return CONF_PATH_DEFAULT;
        }
        File execParentDir = home.getSource().getParentFile();
        File file = new File(execParentDir, CONF_DIR_DEFAULT);
        if (file.exists()) {
            return file.getPath();
        }
        //开发环境main方法启动时
...
//	}
    ...
    public Object load(String path) {
        ...
        File file = new File(getConfDir() + "\\" + path + ".groovy");
        return load(file);
    }
